...
2. **Impact** — duration, scope (which users/systems), quantified where possible (requests failed, revenue/SLA impact — reuse the `observability` SLO/error-budget numbers if the service tracks them).
